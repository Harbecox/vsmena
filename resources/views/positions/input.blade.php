...
@section("main")
  ...
  <form action="{{ action('PositionsController@save') }}" method="POST">
    @if ($subcat->id)
      {{ method_field('PUT') }}
    ...
    <input type="text" placeholder = "менеджер" size="70" name="name" class="source" value="{{ old('name', $subcat->name) }}" required>
    ...
    <select name="restaurants_id">
      @foreach ($rests as $rest)
        <option value="{{ $rest->id }}" @if (old('restaurants_id', $subcat->restaurants_id) == $rest->id)
  ...
  <p><a href="/positions">Список должностей</a></p>
@endsection
